...
export default function AlbumScreen({ navigation }) {
  ...
  const loadPhotos = async () => {
    if (!user?.uid) return;
    try {
      const [albumData, localPhotos] = await Promise.all([
        getUserAlbum(user.uid),
        getAllPhotos(user.uid),
      ]);
      const remotePhotos = albumData.photos || [];
      ...
      const seen = new Set();
      const mergedPhotos = [...remotePhotos, ...normalizedLocal]
        .filter((photo) => {
      ...
      <ScrollView
        ...
        {tagPhotos.slice(0, 6).map(photo => (
          <TouchableOpacity
            ...
            <SafeImage
              source={{ uri: photo.cloudinaryUrl || photo.uri || photo.localUri }}
        ...
        <View style={styles.statsBar}>
          {selectionMode ? (
            ...
            <View style={styles.statsPill}>
              <Ionicons name="images" size={14} color="#FACC15" style={{ marginRight: 4 }} />
